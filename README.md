...
- `/changelog/` and `/changelog.xml` (`src/lib/changelog.ts`) are the public changelog #19 asked for: newly published languages dated by the earliest release in their repos' Door43 release history (`firstReleased`, read by `fetch-catalog.mjs` from the same releases request that backfills assets, and cached in the snapshot for good once known), then translations with a new release in the last 12 months. A language whose history could not be read is named as undated rather than dated by its latest release; `check:routes` asserts that against the served page. English-only at the root like the legal pages — the lines are autonyms, codes and dates — and linked from every footer as "What's new". See `docs/search-visibility.md` → Freshness.
